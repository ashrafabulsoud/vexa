...
Vexa sends meeting audio to one speech-to-text endpoint. By default that endpoint is
OpenAI-compatible (`/v1/audio/transcriptions`). The endpoint can be the bundled faster-whisper
service, Vexa's hosted transcription service, or a self-hosted gateway that implements the same
request shape. **ElevenLabs Scribe is a different dialect** — use
[Use ElevenLabs for STT and TTS](/how-to/elevenlabs) instead of this page.
