...
| **Speak** | sealed | ❌ 404 | proven live at the bot layer (acts.v1 `speak` → TTS → virtual mic), **not wired at the API layer** — tracked in [#514](https://github.com/Vexa-ai/vexa/issues/514). Bot-layer TTS may be OpenAI-compatible or [ElevenLabs](/how-to/elevenlabs) (`TTS_SERVICE_URL` / `TTS_VOICE_ID`); that does not mount this REST surface. |
