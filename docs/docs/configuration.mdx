...
|---|---|---|
| `TRANSCRIPTION_SERVICE_URL` | — | STT service URL, e.g. `http://<gpu-host>:8083` or `https://api.elevenlabs.io`. Give it the **base URL** (canonical) or the full endpoint — every consumer appends the path only when it is missing. OpenAI-compatible backends use `/v1/audio/transcriptions`; an `*.elevenlabs.io` host (or `TRANSCRIPTION_BACKEND=elevenlabs`) uses Scribe `/v1/speech-to-text`. Unset + default `transcribe_enabled=true` → `POST /bots` answers **503** naming the missing keys (not a silent empty transcript). |
| `TRANSCRIPTION_SERVICE_TOKEN` | — | STT auth token — must match the `API_TOKEN` of your `deploy/transcription` unit, a token from `vexa.ai/account`, or an ElevenLabs `xi-api-key`. |
| `TRANSCRIPTION_MODEL` | `whisper-1` (OpenAI-compatible) / `scribe_v2` (ElevenLabs) | STT model id sent on every transcription request. **Set it when the backend validates model ids** — Groq (`whisper-large-v3-turbo`), OpenAI (`whisper-1` / `gpt-4o-transcribe`), ElevenLabs (`scribe_v2` / `scribe_v1`), vLLM/LiteLLM (the served name) — or every request fails `model_not_found`. Ignored by the bundled `deploy/transcription` unit, whose model is its own `MODEL_SIZE`. |
| `TRANSCRIPTION_BACKEND` | inferred from the URL | `elevenlabs` forces the Scribe dialect when the URL hostname is not under `elevenlabs.io` (a private reverse proxy). Unset → OpenAI-compatible unless the host is ElevenLabs. See [Use ElevenLabs for STT and TTS](/how-to/elevenlabs). |
...
## Text-to-speech (bot speak path)

These are forwarded from the runtime onto every spawned bot. They do **not** wire `POST /bots/.../speak` (that route still 404s — [Interactive bots](/interactive-bots)). They power the bot-layer `acts.v1` `speak` path when a speak act arrives.

| Variable | Default | Purpose |
|---|---|---|
| `TTS_SERVICE_URL` | — | TTS base URL. OpenAI-compatible `/v1/audio/speech` by default; an `*.elevenlabs.io` host (or `TTS_BACKEND=elevenlabs`) uses `/v1/text-to-speech/{voice}/stream?output_format=pcm_24000`. Unset → speak is a no-op. |
| `TTS_API_TOKEN` | — | TTS auth token (`X-API-Key` for OpenAI-compatible, `xi-api-key` for ElevenLabs). |
| `TTS_BACKEND` | inferred from the URL | `elevenlabs` forces the ElevenLabs dialect on a private proxy. |
| `TTS_VOICE_ID` | — | Default ElevenLabs voice id when the speak act's `voice` is `auto`. Required for ElevenLabs speak; without it the bot logs a no-op. |
| `TTS_MODEL` | `tts-1` / `eleven_flash_v2_5` | Model id for the configured dialect. |
